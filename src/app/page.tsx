...
const bahasaResponses = {
  ...
  proyek: "Saya telah mengerjakan beberapa proyek, termasuk: Visualisasi Polusi Udara di Jakarta, Desain Basis Data E-Library, Prediksi Churn Pelanggan, Policy Brief tentang Kesadaran Lingkungan, dan lainnya! (Untuk proyek tambahan, kunjungi bagian Proyek). Catatan: Jika Anda menggunakan Android/iPhone, klik 3 garis horizontal di samping tombol Mode Gelap. Jangan ragu untuk menghubungi saya jika Anda ingin membahas proyek-proyek ini lebih detail!",
...
export default function Home() {
  ...
  const handleChatSubmit = (e: React.FormEvent) => {
    ...
    thinkingTimeoutRef.current = setTimeout(() => {
      ...
      } else if (lowerMessage.includes("apa kabar")) {
        const apaKabarResponses = [
          "Saya baik-baik saja, terima kasih! Bagaimana dengan Anda?",
          "Alhamdulillah, saya baik. Anda sendiri bagaimana kabarnya?",
          "Baik sekali! Senang Anda menanyakan kabar saya. Ada yang bisa saya bantu?"
        ];
        response = apaKabarResponses[Math.floor(Math.random() * apaKabarResponses.length)];
      ...
      // Time and date
        ...
        response = "My name is Putra Dinantio Nugroho, a Data Science enthusiast with a background in Political Science from the University of Indonesia.";
        ...
        response = "I don't listen to music, but I'd love to know what kind of music you enjoy!";
                ...
                (lowerMessage.includes('price') || lowerMessage.includes('harga'))) {
        response = "I don't have access to real-time Bitcoin prices. To check the current BTC price, you can visit CoinMarketCap, Binance, or other cryptocurrency tracking websites.";
      } else if ((lowerMessage.includes('ethereum') || lowerMessage.includes('eth')) && 
                (lowerMessage.includes('price') || lowerMessage.includes('harga'))) {
        response = "I don't have access to real-time Ethereum prices. To check the current ETH price, you can visit CoinMarketCap, Binance, or other cryptocurrency tracking websites.";
      } else if ((lowerMessage.includes('usd') || lowerMessage.includes('dollar')) && 
                (lowerMessage.includes('idr') || lowerMessage.includes('rupiah')) && 
                (lowerMessage.includes('rate') || lowerMessage.includes('exchange') || lowerMessage.includes('kurs'))) {
        response = "I don't have access to real-time exchange rates. To check the current USD to IDR exchange rate, you can visit Bank Indonesia's website, XE.com, or other currency exchange rate services.";
      }
